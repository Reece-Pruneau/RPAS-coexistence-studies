...
%d_sep=10; % (m) seperation distance between both networks
n_a=400; %number of monte carlo runs
offset=60; %degrees
...
%Generate n_a terrestrial UE positions per sector
for n=1:19
[UE_c(1:2,1,n,:),outline_c(:,:,n,1)]=randHEX(r_a,BS_c(2,n)+r_a,BS_c(1,n),n_a);
[UE_c(1:2,2,n,:),outline_c(:,:,n,2)]=randHEX(r_a,BS_c(2,n)-0.5*r_a,BS_c(1,n)+0.866*r_a,n_a);
[UE_c(1:2,3,n,:),outline_c(:,:,n,3)]=randHEX(r_a,BS_c(2,n)-0.5*r_a,BS_c(1,n)-0.866*r_a,n_a);
end
...
d_c=NaN(3,19,n_a,3);
phi_c=NaN(3,19,n_a);
theta_c=NaN(3,19,n_a);
AntGain_c_c=NaN(3,19,n_a);
L_c_c=NaN(3,19,n_a);
indoor_c=NaN(3,19,n_a);
...
d_c(s,n,i,:)=[UE_c(1,s,n,i)-BS_c(1,n),UE_c(2,s,n,i)-BS_c(2,n),UE_c(3,s,n,i)-BS_c(3,n)];

%angle between own BS max gain vector and displacement vector in xy plane
phi_c(s,n,i)=acosd(dot(squeeze(d_c(s,n,i,1:2)),BS_cV(s,1:2))./norm(BS_cV(s,1:2))./vecnorm(d_c(s,n,i,1:2)));

%elevation angle
theta_c(s,n,i)=acosd(dot([vecnorm(d_c(s,n,i,1:2)),d_c(s,n,i,3)],[norm(BS_cV(s,1:2)),BS_cV(s,3)])./norm(BS_cV(s,:))./vecnorm(d_c(s,n,i,:))).*d_c(s,n,i,3)./abs(d_c(s,n,i,3));

%Antenna gain to own BS
AntGain_c_c(s,n,i)=F1336(phi_c(s,n,i),theta_c(s,n,i),G_0,tilt);
...
%P1546
L_c_c(s,n,i)=-P452(round(vecnorm(d_c(s,n,i,1:2))+1),1);


if rand>indoor_ratio
indoor_c(s,n,i)=0;
I_dBmUEc(s,n,i)=min([max_dBmUE, M_pusch + P_0 - alpha.*(AntGain_c_c(s,n,i)+L_c_c(s,n,i)+UE_gain+L_feeder+L_body) ]);

else
indoor_c(s,n,i)=1;
I_dBmUEc(s,n,i)=min([max_dBmUE, M_pusch + P_0 - alpha.*(AntGain_c_c(s,n,i)+L_c_c(s,n,i)+UE_gain+L_feeder+L_body+L_entry) ]);

end

if I_dBmUEc(s,n,i)>=max_dBmUE
    I_dBmUEc(s,n,i)=max_dBmUE;
    n_UEmaxC(s,n)=n_UEmaxC(s,n)+1;
end
if I_dBmUEc(s,n,i)<=min_dBmUE
    I_dBmUEc(s,n,i)=min_dBmUE;
...
%P1546
L_b_b(s,n,i)=-P452(round(vecnorm(d_b(s,n,i,1:2))+1),1);


if rand>indoor_ratio
indoor_b(s,n,i)=0;
I_dBmUEb(s,n,i)=min([max_dBmUE, M_pusch + P_0 - alpha.*(AntGain_b_b(s,n,i)+L_b_b(s,n,i)+UE_gain+L_feeder+L_body) ]);

else
indoor_b(s,n,i)=1;
I_dBmUEb(s,n,i)=min([max_dBmUE, M_pusch + P_0 - alpha.*(AntGain_b_b(s,n,i)+L_b_b(s,n,i)+UE_gain+L_feeder+L_body+L_entry) ]);
...
D_c=NaN(3,19,3,19,n_a,3);
PHI_c=NaN(3,19,3,19,n_a);
THETA_c=NaN(3,19,3,19,n_a);
AntGain_c_r=NaN(3,19,3,19,n_a);
L_c_r=NaN(3,19,3,19,n_a);
...
I_c=zeros(3,19,3,19,n_a); %Baseline interference from tertrial network
...
D_c(S,N,s,n,i,:)=[UE_c(1,s,n,i)-BS_b(1,N),UE_c(2,s,n,i)-BS_b(2,N),UE_c(3,s,n,i)-BS_b(3,N)];

%angle between ref BS max gain vector and displacement vector in xy plane
PHI_c(S,N,s,n,i)=acosd(dot(squeeze(D_c(S,N,s,n,i,1:2)),BS_bV(S,1:2))./norm(BS_bV(S,1:2))./vecnorm(D_c(S,N,s,n,i,1:2)));

%elevation angle
THETA_c(S,N,s,n,i)=acosd(dot([vecnorm(D_c(S,N,s,n,i,1:2)),D_c(S,N,s,n,i,3)],[norm(BS_bV(S,1:2)),BS_bV(S,3)])./norm(BS_bV(S,:))./vecnorm(D_c(S,N,s,n,i,:))).*D_c(S,N,s,n,i,3)./abs(D_c(S,N,s,n,i,3));

%Antenna gain to ref BS
AntGain_c_r(S,N,s,n,i)=F1336(PHI_c(S,N,s,n,i),THETA_c(S,N,s,n,i),G_0,tilt);
...
%P1546
L_c_r(S,N,s,n,i)=-P452(round(vecnorm(D_c(S,N,s,n,i,1:2))),1);



if indoor_c(s,n,i)==0
I_c(S,N,s,n,i)=10.^((I_dBmUEc(s,n,i)+AntGain_c_r(S,N,s,n,i)+L_c_r(S,N,s,n,i)+UE_gain+L_body+L_feeder)./10); %mW
elseif indoor_c(s,n,i)==1
I_c(S,N,s,n,i)=10.^((I_dBmUEc(s,n,i)+AntGain_c_r(S,N,s,n,i)+L_c_r(S,N,s,n,i)+UE_gain+L_body+L_feeder+L_entry)./10); %mW
end
...
I_a(S,N,s,n,i)=10.^((I_dBmUEa(s,n,i)+AntGain_a_r(S,N,s,n,i)+L_a_r(S,N,s,n,i)+UE_gain+L_feeder)./10); %mW
 ...
 I_a(S,N,s,n,i)=10.^((I_dBmUEa(s,n,i)+AntGain_a_r(S,N,s,n,i)+L_a_r(S,N,s,n,i)+UE_gain+L_feeder+L_entry)./10); %mW
end   
% 50% LOADING factor
if rand>loading_factor
   I_a(S,N,s,n,i)=0;
...
loss_ref_50th(d)=throughput_loss_map(1,8);
IoN_ref_50th=IoN_map(1,8);

loss_ref_95th(d)=CDF_loss_ref_V(round(0.95*(n_a+1)));
IoN_ref_95th=CDF_IoN_ref_V(round(0.95*(n_a+1)));

[CDF_loss_ref_P_t, CDF_loss_ref_V_t]=homemade_ecdf(reshape(bitloss_t(1,8,:),[1,n_a]));
loss_ref_50th_t(d)=throughput_loss_map_t(1,8);
loss_ref_95th_t(d)=CDF_loss_ref_V_t(round(0.95*(n_a+1)));
